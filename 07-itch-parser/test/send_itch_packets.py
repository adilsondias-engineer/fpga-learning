...
class ITCHMessageGenerator:
    ...
    def system_event(self, event_code='O'):
        ...
        msg.extend(struct.pack('>Q', self._get_timestamp())[2:])  # Timestamp (6 bytes)
        msg.append(ord(event_code))                 # Event code
        
        assert len(msg) == 12, f"System Event message wrong size: {len(msg)}"
        return bytes(msg)
    
    def stock_directory(self, symbol='AAPL    '):
        """
        ...
        msg.append(ord('N'))                        # Market category (N=NYSE)
        msg.append(ord('N'))                        # Financial status
        msg.extend(struct.pack('>I', 100))          # Round lot size
        msg.append(ord('N'))                        # Round lots only
        msg.append(ord('N'))                        # Issue classification
        msg.extend(b'  ')                           # Issue sub-type (2 bytes)
        msg.append(ord('Y'))                        # Authenticity
        msg.append(ord('N'))                        # Short sale threshold
        msg.append(ord('N'))                        # IPO flag
        msg.append(ord('N'))                        # LULD ref price tier
        msg.append(ord('N'))                        # ETP flag
        msg.extend(struct.pack('>I', 0))            # ETP leverage factor
        msg.append(ord('N'))                        # Inverse indicator
...
def main():
    ...
    if args.test:
        if args.test == 'lifecycle':
            ...
            test_single_message(iface, args.target, args.port, args.test)
    elif args.sequence:
